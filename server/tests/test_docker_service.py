...
class TestDockerVolumeValidation:
    ...
    async def test_pvc_volume_not_found_rejected(self, mock_docker):
        """PVC backend with non-existent Docker named volume should be rejected when auto-create is disabled."""
        mock_client = MagicMock()
        mock_client.containers.list.return_value = []
        ...
        cfg = _app_config()
        cfg.storage = StorageConfig(volume_auto_create=False)
        service = DockerSandboxService(config=cfg)
    ...
    def test_pvc_volume_auto_created_when_not_found(self, mock_docker):
        """PVC backend auto-creates Docker named volume when volume_auto_create is enabled."""
        mock_client = MagicMock()
        mock_client.containers.list.return_value = []
        ...
        cfg = _app_config()
        cfg.storage = StorageConfig(volume_auto_create=True)
        service = DockerSandboxService(config=cfg)
